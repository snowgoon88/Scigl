...
// VIEW
LoggedVectorSciglPtr _vec_obs1 = LoggedVectorSciglPtr( new LoggedVectorScigl(_vec_model));
LoggedVectorSciglPtr _vec_obs2 = LoggedVectorSciglPtr( new LoggedVectorScigl(_vec_model));
...
void init_graphic()
{
  ...
  _vec_model->attach_observer( _vec_obs1 );
  _vec_obs1->_dim = 1;
  _vec_model->attach_observer( _vec_obs2 );
  _vec_obs2->set_thickness( 2.5 );
  _vec_obs2->_dim = 2;
  //_vec_obs2->_abs = -1; // t of LoggedVector
  //_vec_obs2->_abs = 0; // v(0) of LoggedVector
  ...
  // Add a system coordinate
  PlaneCoordPtr coord_rec = PlaneCoordPtr (new PlaneCoord());
  coord_rec->set_title_coord1( "LastIter" );
  Range rg_x_rec( 0, 200, 5, 5*5);
  coord_rec->set_range_coord1( rg_x_rec );
  coord_rec->set_title_coord2( "Avg. Reward" );
  Range rg_y_rec( -2.0, 2.0, 5, 5*5);
  coord_rec->set_range_coord2( rg_y_rec );
  Position pos_rep_rec( -0.45, -0.45, 0, 0);
  coord_rec->set_position( pos_rep_rec );
  Size size_rep_rec( 1.5, 1.5, 1, 0.0);
  coord_rec->set_size( size_rep_rec );
  _scene->add ( coord_rec );

  _vec_obs1->attach_to_coord( coord_rec );
  _scene->add( _vec_obs1 );
  _vec_obs2->attach_to_coord( coord_rec );
