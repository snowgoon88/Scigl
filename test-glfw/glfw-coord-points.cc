...
int main (int argc, char **argv)
{
  ...
  PlaneCoordPtr coord = PlaneCoordPtr (new PlaneCoord());
  coord->set_title_coord1( "X" );
  Range rg_x( 0, 1, 4, 4*5);
  coord->set_range_coord1( rg_x );
  coord->set_title_coord2( "Y" );
  Range rg_y( -1, 1, 4, 4*5);
  coord->set_range_coord2( rg_y );
  Position pos_rep( -0.5, -0.5, 0, 0);
