...
PlaneCoordPtr plane;
float slide_max1 = 1.0;
float slide_max2 = 1.0;
...
void on_key_pressed( int key, int action)
{
  ...
  // s or S : advance sliding
    ...
    slide_max1 += 0.05; slide_max2 += 0.1;
    plane->set_slide_max1( slide_max1 );
    plane->set_slide_max2( slide_max2 );
  }
  // t or T : toggle between sliding or not
  else if( ((key == 116) || (key == 84)) && (action == GLFW_PRESS)) {
    // NOT WORKING NOW : set_sliding_coord2( true )
    if( plane->get_sliding_coord1() == true) {
      plane->set_sliding_coord1( false );
      std::cout << "Sliding OFF\n";
    }
    else {
      plane->set_sliding_coord1( true );
      std::cout << "Sliding ON\n";
    }
  }
}
...
std::cout << "Init GLEW\n";
  ...
  // Plane Coord
  plane = PlaneCoordPtr (new PlaneCoord() );
  // NOT WORKING RIGHT NOW
  //plane->set_sides_type( PlaneCoord::AXIS, PlaneCoord::LINE, PlaneCoord::AXIS, PlaneCoord::AXIS );
  plane->set_range_coord1( Range(-1, 1, 5, 5*4));
  plane->set_position( -0.5, -0.5 );
  scene->add (plane);

  scene->set_orientation( 30, -30);
  scene->set_zoom(1.9);
